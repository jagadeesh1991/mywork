...
def get_stats_full():
    ...
    try:
       ...
       # userinfo = pwd.getpwuid(st.ST_UID)
        ...
        response_dict.update({'Last Access Time': st.st_atime})
        response_dict.update({'Last Modification Time': st.st_mtime})
        response_dict.update({'Last Change Time': st.st_ctime})
